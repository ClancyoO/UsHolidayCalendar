...
| U4 | 一天多个节日时单元格内容溢出，显示混乱不美观 | 中 | [index.html:415-437](file:///workspace/index.html#L415-L437) |
...
#### 2.5 多节日显示优化（参考 iOS 日历风格）
- **文件**: `index.html`
- **操作**: 参考 iOS 日历的多事件显示方式，所有节日全部展示但保持美观：
  - **桌面端**：每个节日显示为紧凑的彩色圆角条（pill），文字左对齐，节日条之间间距2px，单元格高度自适应内容（移除固定 `min-height: 110px`，改为 `min-height: 80px` + `height: auto`）
  - **手机端迷你日历**：节日显示为小彩色圆点（每行最多3个），点击日期后在日历下方展开该日所有节日的详情面板（类似 iOS 日历点击日期后下方显示事件列表）
  - **手机端列表视图**：所有节日完整展示，无需截断
  - 节日条样式优化：圆角更小（`rounded` → `rounded-sm`），内边距更紧凑（`p-1.5` → `px-1.5 py-0.5`），字号保持 `text-xs` 但行高压缩
- **原因**: 用户要求所有节日都显示，不能截断。iOS 日历的做法是在格子内尽可能显示彩色事件条，超出部分通过点击展开查看，兼顾信息完整性和视觉美观
...
#### 3.3 日历下方节日详情面板（iOS 风格）
- **文件**: `index.html`
- **操作**: 在日历网格下方添加一个可展开的详情面板区域：
  - 点击有节日的日期格子时，日历下方滑出该日所有节日的列表
  - 列表中每个节日显示完整信息（彩色类型标记 + 中文名 + 英文名）
  - 点击节日项可打开模态框查看详情
  - 再次点击同一日期或点击其他区域可收起面板
  - 面板有平滑的展开/收起动画
- **原因**: 参考 iOS 日历交互模式，解决多节日显示问题，同时保持日历格子简洁
